...
import os
import sys
sys.path.append("E:\BHHJ\Code\Strats_Interface\\")
	...
	def query_sql_procedure(sql_script_IN, table_res_IN = 1):
		if table_res_IN > 0:
			...
			con = pyodbc.connect(driver='{SQL Server Native Client 11.0}',
				#driver='{SQL Server}',
				host = Config.sql_server, database=Config.cracked_tables_prod_db,trusted_connection='yes',autocommit=True)
			cursor = con.cursor()
			...
			con = pyodbc.connect(driver='{SQL Server Native Client 11.0}', 
				#driver='{SQL Server}',
				host = Config.sql_server, database=Config.cracked_tables_prod_db,trusted_connection='yes')
			cursor = con.cursor()
